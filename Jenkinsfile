pipeline {
    ...
    environment {
        ...
        BUCKET_NAME="dh-pi-devopsbarbara-dev"
        REGION="us-east-1" 
        PERMISSION=""
            ...
            stages {
                ...
                stage('Docker push') {
                    steps {
                        ...
                        script {
                            docker.withRegistry('https://329753855425.dkr.ecr.us-east-1.amazonaws.com/', 'ecr:us-east-1:ecraws') {
            ...
            steps { 
                script {
                    if(env.GIT_BRANCH=='origin/master'){
                        ...
                        sh "docker run -d --name app_homolog -p 3000:3000 329753855425.dkr.ecr.us-east-1.amazonaws.com/digitalhouse-devops-app:latest"
                        sh "docker ps"
                        sh 'sleep 10'
                        sh 'curl http://127.0.0.1:3000/api/v1/healthcheck'
